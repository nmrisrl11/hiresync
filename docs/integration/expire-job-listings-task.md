...
## Overview
To automatically manage the lifecycle of job listings, we utilize the `@nestjs/schedule` package. This allows us to run a scheduled background task that checks for job listings that have passed their `expiresAt` date and safely transitions them to a `CLOSED` (or `EXPIRED`) state. 
...
**Location:** `src/recruitment/infrastructure/tasks/expire-job-listings.task.ts`
...
## Architecture Flow
1. **Trigger:** `ExpireJobListingsTask` fires at midnight.
...
5. **Event Dispatch:** `DomainEventPublisher` fires the event.
6. **Side Effects:** The `JobListingClosedListener` catches the event and queues the notification email via BullMQ.

## Module Registration
The Task service must be registered as a provider within the module that owns it.
...
## Testing Locally
To test the task locally without waiting for midnight, temporarily change the `@Cron` decorator to run frequently:
